...
**Step 3 is where the wall clock goes, and it is one pipeline, not two phases.** Authoring is by far the most expensive thing the app does — a batch runs 30-60s against a ~1s independent solve — so every authoring call for a set is in flight at once (`generateProblems` packs the requested format mix into `PROBLEMS_PER_CALL` chunks), and `onBatch` starts verifying each batch the moment it lands rather than waiting for the slowest one. Three consequences worth knowing before touching it:
...
**The free tier meters requests per day, not tokens, and that is what the call layout is for.** `GenerateRequestsPerDayPerProjectPerModel` is 20 on `gemini-3.5-flash` and 500 on each flash-lite, while a call costs roughly the same whatever it carries — so the number of *calls* a set makes is the only thing that decides how many sets a day exist. Both halves are packed accordingly, and they are packed for opposite reasons:

- **Authoring packs as hard as it can.** The count is still split across generation kinds first, because `graph` is three unrelated tasks, but the mix is then packed into as few calls as it fits in rather than one call per kind. That split-before-chunk ordering used to make the *number of formats* set the request count: six problems across six formats went out as six calls of one problem each, and `PROBLEMS_PER_CALL` never bound. Measured against production, a six-problem set cost twelve requests and now costs three.
- **Verification packs gently** (`SOLVES_PER_CALL`, 5). The independent solve is the quality gate everything else rests on, so the limit is about context rather than throughput: a long shared context is where a solver starts pattern-matching between problems instead of solving each, and one unusable response should cost a few re-solves rather than a whole set's verification. `SolvedBatchSchema` carries `problem_number` for the same reason — pairing results to problems positionally would mis-pair the entire tail of a batch the moment a model returned four results for five problems, and a problem judged against its neighbour's solution is the worst failure available here. An unclaimed problem comes back `null` and is re-solved alone.
