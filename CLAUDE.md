...
- **`--equivalence` is a separate flag from `--verify` because substituting Claude costs almost nothing there and a great deal here.** The blind solve has to be a stranger — it exists to disagree, and an author and a solver sharing a blind spot agree wrongly. Equivalence takes *both* answers as input, so it was never blind, and it asks a language question rather than a second opinion. Bundling the two under one flag would have priced the cheap substitution at the expensive one's rate — and since `defer` drops every prose answer, that is the difference between a no-provider run that writes all four styles and one that writes only `word`. `adjudicate.ts` batches the open pairs into one subprocess between two `judgeAll` passes rather than spawning per question; re-judging is free because `solverGates` makes no call of its own. Every failure path there yields *fewer verdicts*, never wrong ones — an unanswered pair stays deferred and drops its problem, which is exactly what `defer` does, so nothing degrades below the setting it replaces.
- **`--author-model` and `--check-model` split what `--model` sets together**, resolved in the CLI so nothing downstream knows a shorthand existed. Authoring is the expensive half and the one worth a large model; re-solving a stated problem and comparing two answers are not. It also recovers a little of what `--verify claude` gives up, since differently-trained models fail in different places — a little, not the structural independence Gemini gets for free.
